...
```{r project.setup, echo=TRUE, results='hide', message=FALSE, warning=FALSE, error=FALSE}
#Import libraries
library(tidyverse)
library(here)
library(knitr)
...
```{r, echo = FALSE, fig.height = 3.5, fig.width = 10, fig.cap = "Monthly Ozone (2018)", fig.align = "left"}
#boxplot of distribution of ozone levels by month for 2018 records
ozone_boxplot_2018 <-
  epa_data %>% 
  filter(Year == 2018, !is.na(Ozone)) %>%
  ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb),
             y = Ozone)) + 
  geom_boxplot() +
  theme(axis.title.x = element_blank()) +
  labs(y = "Ozone (ug/m^3)", title = "Monthly Ozone: 2018") 
print(ozone_boxplot_2018) 
```

```{r, echo = FALSE, fig.height = 3.5, fig.width = 10, fig.cap = "Monthly Ozone (2019)", fig.align = "left"}

ozone_boxplot_2019 <-
  epa_data %>% 
  filter(Year == 2019, !is.na(Ozone)) %>%
  ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb),
             y = Ozone)) +
  geom_boxplot() +
  theme(axis.title.x = element_blank()) +
  labs(y = "Ozone (ug/m^3)", title = "Monthly Ozone: 2019")
print(ozone_boxplot_2019) 
...
## Git Repository
https://github.com/lexiwn17/EDE_Spring2026

"I did not use AI in the completion of this assignment -Lexi N 3/2/26"
