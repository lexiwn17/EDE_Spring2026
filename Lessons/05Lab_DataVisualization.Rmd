...
#List unique values
unique(PeterPaul.chem.nutrients$month) #months may-dec


#Plot how many Lake observations occur each month (bar plot)
ggplot(PeterPaul.chem.nutrients, aes(x=month)) + geom_bar()
#not a great plot because the x axis is every 3 months
#since they are integers not factors
...
#add levels 1-12 per month
PeterPaul.chem.nutrients%>%
  mutate(month_f = factor(month, level=1:12)) %>%
  ggplot(aes(x=month_f)) +
  geom_bar() 
...
PeterPaul.chem.nutrients%>%
  mutate(month_f = factor(month, level=1:12, labels=month.abb)) %>%
  ggplot(aes(x=month_f)) +
  geom_bar()  +
...
ex_1_plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0.00) %>% 
    ggplot(aes(
      x=daynum,
      y=temperature_C,
  ...
  ylim(-10, 40) +
  geom_point() +
  ggtitle("Surface Temps by Day of Year")
print(ex_1_plot)
...
ex_2_plot <- PeterPaul.chem.nutrients %>% 
    ggplot(aes(
      x=sampledate,
      y=temperature_C,
      color=depth)) +
