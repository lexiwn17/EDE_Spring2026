...
```{r}
#Exercise 1:
EPAair %>% #can pipe with shift-control-M
  ggplot(aes(x=Ozone, fill=factor(Year))) +
  geom_density(alpha=0.4) + #generates smooth density estimate plot, alpha for transparency
  scale_fill_manual(values=c('blue','yellow')) #to manually select colors +
  labs(
...
#looking at distribution of air quality of ozone throughout the 2 years
EPAair %>% 
  ggplot(aes(x=factor(Year), y=Ozone)) +
  geom_boxplot() +
  #stat_summary adds a new layer to the plot to annotate it
  stat_summary(
    geom = "text",
    fun = max,
    label = "max",
...
#now change it to a violin plot
EPAair %>% 
  drop_na(Ozone) %>% 
  ggplot(aes(x=factor(Year), y=Ozone)) +
  geom_violin() +
...
Pm2.5ozone.regression <- 
  lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(Pm2.5ozone.regression)
#interpretation: 
#estimate: when ozone is 0, we will measure PM2.5 of 16
#R squared: ozone only explains 13% of the variability in PM2.5
