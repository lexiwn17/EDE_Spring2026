...
```{r dateorigin}
#Check if "1970-01-01" is your origin date.this is a fairly popular origin date
lubridate::origin
...
sites <- read.csv(
  ...
  stringsAsFactors = T, #we generally want to treat character columns as categories
  #they will act as grouping variables, so we want them as factors
  colClasses=c('site_no'='factor')
  )
dim(sites) #gives number of rows and columns
...
sites$dow <- wday(sites$date) #highlight wday and hit F1 to find it is command to get weekday
...
#See that the levels are updated
levels(sites$dow)
#these will not appear in our data frame but are updated "behind the scenes"
