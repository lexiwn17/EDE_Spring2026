...
> Add notes here: give insight into climate change
...
NTL.phys.data.ex3 <- filter(NTL.phys.data, lakename == 'Tuesday Lake' & year4 %in% c(1990:1999))
#recall | is or so it would take anything that met at minimum one of the criteria
...
> Answer: row numbers are not tied to a characteristic of the data itself and therefore are less precise
...
NTL.phys.data.depth.descending2 <- arrange(NTL.phys.data, desc(temperature_C))
# Which dates, lakes, and depths have the highest temperatures?
#East Long & Hummingbird
...
#split/apply/combine
#use the group function to split data
demo <- NTL.phys.data %>% 
  filter(depth == 0) %>% 
  group_by(year4) %>% #split
  combine(year4, depth)
