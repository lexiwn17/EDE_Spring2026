...
upper_neuse <- huc8_sf_2 %>%
  filter(huc8_sf_2$SUBBASIN == "Upper Neuse")


#Create a map
map_base <- mapview(huc8_sf, col.regions = "lightgray", legend = FALSE)

# Overlay Upper Neuse in yellow with blue border
map_upper <- mapview(upper_neuse, col.regions = "yellow", alpha = 0.5, lwd = 2, color = "blue", legend = FALSE)
...
wind_energy <- read_csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', col_types = cols(
    HUC_12 = col_factor(),      # HUC12 as factor
    .default = col_double()     # other columns as numeric
  )
) 
%>%
  # Create HUC_8 by taking first 8 characters of HUC_12
  mutate(HUC_8 = substr(HUC_12, 1, 8)) %>%
  # Group by HUC_8 and sum wind energy
  group_by(HUC_8) %>%
  summarize(total_wind_energy = sum(wind_energy, na.rm = TRUE))

# join to HUC8 features
huc8_sf_wind <- huc8_sf %>%
  left_join(wind_energy, by = c("HUC8" = "HUC_8"))  # assuming your HUC8 column is named HUC8

# View the outputs
head(huc8_sf_wind)
mapview(huc8_sf_wind, zcol = "total_wind_energy", legend = TRUE)
...
triHUC8 <- HUC8_sf_utm %>% 
  filter(st_intersects(., triCo, sparse = FALSE))

# Clip the HUC_8s to the Triangle counties
triHUC8_clip <- st_intersection(triHUC8, triCo)
