...
More info: <https://doi-usgs.github.io/dataRetrieval/>

```{r using.the.dataRetrieval.package}
#Identify gauge to download
siteNo <- 'USGS-02087500' 
siteNos <- c('USGS-02087500','USGS-02085070')

#Identify parameter of interest: 
pcode = '00060' #discharge (cfs)

#Load in the data using the package's "read_waterdata_continuous" function
neuse <- read_waterdata_continuous(
  monitoring_location_id = siteNos,
  parameter_code = pcode,
  time = 'P7D',
)

#Plot
ggplot(neuse, aes(x=time,y=value,color=monitoring_location_id)) + 
  geom_line() +
...
#Plot population (log tranformed) for NC
ggplot(nc_pop, aes(fill = log(estimate))) +
  geom_sf() +
  coord_sf(crs = 4326) +
  scale_fill_viridis("Log(Population)",option = "viridis")+
  labs(title = 'NC Population (log transformed)') +
